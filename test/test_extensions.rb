...
class TestExtensions < Minitest::Test
  ...
  def test_bad_extension_specifications
    assert_raises(TypeError) { Markly.render_html(@markdown, :DEFAULT, 'nope') }
    assert_raises(TypeError) { Markly.render_html(@markdown, :DEFAULT, ['table']) }
    assert_raises(ArgumentError) { Markly.render_html(@markdown, :DEFAULT, %i[table bad]) }
  end
  ...
  def test_table_prefer_style_attributes
    assert_equal(<<~HTML, Markly.render_html(<<~MD, :TABLE_PREFER_STYLE_ATTRIBUTES, %i[table]))
      <table>
      <thead>
      <tr>
